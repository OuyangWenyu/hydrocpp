...
C或C++程序的编译运行更加底层基础一些，所以相比python等语言，入手的时候会难一点，这里就借了解CMake的机会，记录下这方面的基本概念。
...
我们日常生活的每一个方面都有计算机软件的存在：它可以为我们触发闹钟，提供通信、银行、天气、公交、日历、会议、旅行、相册、电视、音乐流、社交媒体、晚餐和电影预定等服务信息。

软件内部具有一定的层次结构：应用程序是基于框架构建，框架基于开发库，开发库使用更小的库或可执行文件（也还有更小的组件）。开发库和可执行文件通常需要使用源代码进行构建。我们通常只看到最外层，但软件内部需要仔细组织和构建。

为了简化日常软件的形成过程，需要提供一组工具，可以在不同平台上配置、构建、测试和部署项目，CMAKE就是这样的工具。
...
编写能够在许多不同平台上本地、可靠并高效地运行的软件执行计算，对于学科及工业界都至关重要。软件构建系统就是这项任务的中心。它们是软件开发生命周期管理的关键部分：从孵化和原型开发到测试，一直到打包、部署和发布。

CMake旨在帮助管理这些操作，因此如果希望使用CMake管理构建系统，或者希望能够理解，并能修改其他人编写的CMake代码，那么就有必要了解CMAKE工作的基本过程。

没有必要一点点系统地学习CMAKE，还是得有个实际的任务作为指引，否则很难理解CMAKE每步的动机意图，也就很难理解其中逻辑了，所以这里仅给出一些基本概念的补充，其他的内容在后续的实践中再一点点加（比如把VIC模型揉到自己的项目里）
